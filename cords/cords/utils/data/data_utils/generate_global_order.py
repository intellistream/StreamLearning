    ...
    if submod_function in ['gc', 'fl', 'logdet', 'disp_min', 'disp_sum']:
        ...
        for cluster in clusters:
            ...
            if submod_function == 'disp_min_pc':
                clustergreedyList = obj.maximize(budget=cluster_embeddings.shape[0] - 1, optimizer='NaiveGreedy',
                                                 stopIfZeroGain=False,
                                                 stopIfNegativeGain=False, verbose=False)
            else:
                clustergreedyList = obj.maximize(budget=cluster_embeddings.shape[0] - 1, optimizer='LazyGreedy',
                                                 stopIfZeroGain=False,
                                                 stopIfNegativeGain=False, verbose=False)
            rem_elem = list(set(range(cluster_embeddings.shape[0])).difference(set([x[0] for x in clustergreedyList])))[
                0]
            ...
            clustered_r2 = np.nonzero(
                clustered_dist <= max(1e-5, clustered_dist.mean() - r2_coefficient * clustered_dist.std()))
    ...
    if submod_function not in ['supfl', 'gc_pc', 'logdet_pc', "disp_min_pc", "disp_sum_pc"]:
        for _ in range(n_subsets):
            ...
            if submod_function == 'disp_min':
                ...
                subset = obj.maximize(budget=budget, optimizer='LazierThanLazyGreedy', epsilon=0.001,
                                      stopIfZeroGain=False,
                                      stopIfNegativeGain=False, verbose=False)
        ...
        for _ in range(n_subsets):
            ...
            for cluster in cluster_idxs.keys():
                ...
                if metric == 'rbf_kernel':
                    ...
                    normalized_embeddings = cluster_embeddings / np.linalg.norm(cluster_embeddings, axis=1,
                                                                                keepdims=True)
                    clustered_sijs = get_dot_product(normalized_embeddings)
                    if submod_function in ["disp_min_pc", "disp_sum_pc"]:
                        clustered_sijs = (clustered_sijs - np.min(clustered_sijs)) / (
                ...
                if submod_function in ['disp_min_pc', 'gc_pc']:
                    ...
                    if per_cls_budget[cluster_idx] == cluster_embeddings.shape[0]:
                        clustergreedyList = obj.maximize(budget=per_cls_budget[cluster_idx] - 1,
                                                         optimizer='StochasticGreedy',
                                                         stopIfZeroGain=False, stopIfNegativeGain=False, epsilon=0.1,
                                                         verbose=False, show_progress=True, costs=None,
                                                         costSensitiveGreedy=False)
                        rem_elem = list(
                            set(range(cluster_embeddings.shape[0])).difference(set([x[0] for x in clustergreedyList])))[
                            0]
                        rem_gain = clustergreedyList[-1][1]
                        clustergreedyList.append((rem_elem, rem_gain))
                    else:
                        clustergreedyList = obj.maximize(budget=per_cls_budget[cluster_idx],
                                                         optimizer='StochasticGreedy',
                                                         stopIfZeroGain=False, stopIfNegativeGain=False, epsilon=0.1,
                                                         verbose=False, show_progress=True, costs=None,
                                                         costSensitiveGreedy=False)
...
def generate_text_similarity_kernel(dataset, model, stats=True, seed=42, data_dir='../data', device='cpu'):
    ...
    if not os.path.exists(os.path.join(os.path.abspath(data_dir), dataset + '_' + model + '_train_embeddings.pkl')):
        ...
        store_embeddings(os.path.join(os.path.abspath(data_dir), dataset + '_' + model + '_train_embeddings.pkl'),
                         train_embeddings)
    else:
        # Load the embeddings from disc
        train_embeddings = load_embeddings(
            os.path.join(os.path.abspath(data_dir), dataset + '_' + model + '_train_embeddings.pkl'))
...
def generate_image_global_order(dataset, model, submod_function, metric, kw, r2_coefficient, knn, seed=42,
    ...
    if not (os.path.exists(os.path.join(os.path.abspath(data_dir),
                                        dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                                kw) + '_global_order.pkl')) and
            os.path.exists(os.path.join(os.path.abspath(data_dir),
                                        dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                            r2_coefficient) + '_global_r2.pkl')) and
            os.path.exists(os.path.join(os.path.abspath(data_dir),
                                        dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                            knn) + '_global_knn.pkl'))):
        global_order, global_knn, global_r2, cluster_idxs = compute_global_ordering(train_embeddings,
                                                                                    submod_function=submod_function,
                                                                                    kw=kw,
                                                                                    r2_coefficient=r2_coefficient,
                                                                                    knn=knn, train_labels=train_labels,
                                                                                    metric=metric)
        dict2pickle(os.path.join(os.path.abspath(data_dir),
                                 dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                     kw) + '_global_order.pkl'),
                    {'globalorder': global_order, 'cluster_idxs': cluster_idxs})
        dict2pickle(os.path.join(os.path.abspath(data_dir),
                                 dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                     r2_coefficient) + '_global_r2.pkl'),
                    {'globalr2': global_r2, 'cluster_idxs': cluster_idxs})
        dict2pickle(os.path.join(os.path.abspath(data_dir),
                                 dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                     knn) + '_global_knn.pkl'), {'globalknn': global_knn, 'cluster_idxs': cluster_idxs})
    else:
        global_order = pickle2dict(os.path.join(os.path.abspath(data_dir),
                                                dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                                    kw) + '_global_order.pkl'), 'globalorder')
        cluster_idxs = pickle2dict(os.path.join(os.path.abspath(data_dir),
                                                dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                                    kw) + '_global_order.pkl'), 'cluster_idxs')
        global_r2 = pickle2dict(os.path.join(os.path.abspath(data_dir),
                                             dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                                 r2_coefficient) + '_global_r2.pkl'), 'globalr2')
        global_knn = pickle2dict(os.path.join(os.path.abspath(data_dir),
                                              dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                                  knn) + '_global_knn.pkl'), 'globalknn')
    return global_order, global_knn, global_r2, cluster_idxs


def generate_text_global_order(dataset, model, submod_function, metric, kw, r2_coefficient, knn, seed=42,
                               data_dir='../data', device='cpu'):
    # Assertion Check:
    assert (dataset in list(SENTENCE_MAPPINGS.keys())) and (dataset in list(LABEL_MAPPINGS.keys())), \
        "Please add the SENTENCE and LABEL column names to the SENTENCE_MAPPING and LABEL_MAPPINGS dictionaries in generate_global_order.py file."
...
def generate_image_stochastic_subsets(dataset, model, submod_function, metric, kw, fraction, n_subsets, seed=42,
                                      data_dir='../data', device='cpu'):
    # Load Dataset
    train_dataset = load_dataset(dataset, data_dir, seed)
    ...
    if not os.path.exists(os.path.join(os.path.abspath(data_dir),
                                       dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                               kw) + '_' + str(fraction) + '_stochastic_subsets.pkl')):
        stochastic_subsets = compute_stochastic_greedy_subsets(train_embeddings, submod_function, train_labels, kw,
                                                               metric, fraction, n_subsets=n_subsets)
        dict2pickle(os.path.join(os.path.abspath(data_dir),
                                 dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                     kw) + '_' + str(fraction) + '_stochastic_subsets.pkl'),
                    {'stochastic_subsets': stochastic_subsets})
    else:
        stochastic_subsets = pickle2dict(os.path.join(os.path.abspath(data_dir),
                                                      dataset + '_' + model + '_' + metric + '_' + submod_function + '_' + str(
                                                          kw) + '_' + str(fraction) + '_stochastic_subsets.pkl'),
                                         'stochastic_subsets')
...
def generate_text_stochastic_subsets(dataset, model, submod_function, metric, kw, fraction, n_subsets, seed=42,
                                     data_dir='../data', device='cpu'):
    # Assertion Check:
    assert (dataset in list(SENTENCE_MAPPINGS.keys())) and (dataset in list(LABEL_MAPPINGS.keys())), \
        "Please add the SENTENCE and LABEL column names to the SENTENCE_MAPPING and LABEL_MAPPINGS dictionaries in generate_global_order.py file."
