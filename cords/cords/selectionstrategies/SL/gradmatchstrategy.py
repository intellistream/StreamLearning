...
class GradMatchStrategy(DataSelectionStrategy):
    ...
    def select(self, valid_out_dim, last_valid_out_dim, budget, model_params):
        ...
        if self.selection_type == 'PerClass':
            ...
            self.compute_gradients(self.valid, valid_out_dim=valid_out_dim, last_valid_out_dim=last_valid_out_dim,
                                   perBatch=True, perClass=False)
            idxs = []
            gammas = []
            trn_gradients = self.grads_per_elem
            if self.valid:
        ...
        # self.logger(f"Random points added: {diff}")
        ...
        # self.logger(f"GradMatch algorithm Subset Selection time is: {omp_end_time - omp_start_time:.4f}")
