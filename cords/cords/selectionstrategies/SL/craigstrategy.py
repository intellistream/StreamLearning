...
class CRAIGStrategy(DataSelectionStrategy):
    ...
    def compute_score(self, valid_out_dim, last_valid_out_dim, model_params, idxs):
        """
        ...
        subset_loader = self.trainloader
        ...
        if self.if_convex:
            ...
            embDim = self.model.feat.num_features
            for batch_idx, (inputs, targets) in enumerate(subset_loader):
                ...
                self.model.eval()
                out, l1 = self.model(inputs, gradmatch=True)
                out = out[:, :valid_out_dim]
                out[:, :last_valid_out_dim] = -float('inf')
                self.model.train()
                ...
                # use this to replace torch.no_grad()
    ...
    def select(self, valid_out_dim, last_valid_out_dim, budget, model_params):
        """
        ...
        if self.selection_type == 'PerClass':
            for i in range(self.num_classes):
                ...
                self.compute_score(valid_out_dim, last_valid_out_dim, model_params, idxs)
