...
def data_receiver(data_queue, end_sign, all_train_dataset, stream_batch_size=100, interval=10):
    start_index = 0
    train_dataset = all_train_dataset.get()
    num_stream = int(len(train_dataset) // stream_batch_size) + 1
    stream_idx = 0
    while True:  # Generate stream data until the end of the dataset is reached
        end_index = start_index + stream_batch_size
        stream_idx += 1
        if end_index > len(train_dataset) - 1:
            end_index = len(train_dataset) - 1
            end_sign.value = 1

        if data_queue.empty():
            logger.info(f'New stream data [{stream_idx}/{num_stream}] arrive...')
            ...
            logger.info(f'Accumulate new stream [{stream_idx}/{num_stream}] data...')
            _ = data_queue.get()
            index.extend(list(range(start_index, end_index)))
            stream_dataset = Subset(train_dataset, index)
            data_queue.put(stream_dataset)
...
def model_trainer(data_queue, end_sign, all_train_dataset):
    ...
    while True:
        if not data_queue.empty():
            ...
            logger.info(f'Train stream data...')
            ...
            trainer.accelerator.state.deepspeed_plugin.hf_ds_config.config["scheduler"]["params"]["total_num_steps"] = 'auto'
            trainer.accelerator.state.deepspeed_plugin.hf_ds_config.config["scheduler"]["params"]["warmup_num_steps"] = 'auto'
            ...
            for log in trainer.state.log_history:
                if 'train_runtime' in log:
                    ...
                    logger.info(f'Stream len of {len(train_dataset)} takes runtime: {stream_runtime[str(len(train_dataset))]} seconds')
...
if __name__ == "__main__":
    ...
    interval = 200  # Data acquiring time interval
    stream_batch_size = 200  # Data acquiring numberf
