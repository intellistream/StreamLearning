...
def compute_metrics(metric_names, metric_fns, *args, **kwargs):
    all_metrics = {}
...
class RetrievalMetric:
    ...
    @classmethod
    def pickle_get_metric_fn(cls, metric_names, **kwds):
        assert isinstance(metric_names, list) or isinstance(metric_names,
                                                            tuple), "You must pass metric_names in a list or tuple!"

        # get all methods
        all_implemented_fns = {x[0]: getattr(cls, x[0]) for x in inspect.getmembers(cls, predicate=inspect.isfunction)
                               if not x[0].startswith("_")}

        # Wrap the metric functions with the additional parameters
        wrapped_fns = {name: partial(fn, **kwds) for name, fn in all_implemented_fns.items()}

        # Return a function that can compute the metrics based on the names
        return partial(compute_metrics, metric_names, wrapped_fns)
