...
from buffer.buffer_utils import maybe_cuda, get_grad_vector, cosine_similarity

class GSSGreedyUpdate(object):
    def __init__(self, config):
        super().__init__()
        # the number of gradient vectors to estimate new samples similarity, line 5 in alg.2
        self.mem_strength = 20
        self.gss_batch_size = config.gss_batch_size
        self.buffer_score = maybe_cuda(torch.FloatTensor(config.mem_size).fill_(0))
